...
/** The team list. Click any user to open their full page — details,
 *  performance, assigned leads, activity, and (admin) profile editing
 *  including batch-ID assignment. */
export function Users() {
  ...
  const createUser = async (e: FormEvent) => {
    ...
    setError('');
    try {
      const created = await post<{ id: number }>('/users', { ...form, roleId: Number(form.roleId) });
      setShowNew(false);
      setForm({ name: '', email: '', password: '', roleId: '' });
      setMsg('User created — opening their page to assign a batch ID.');
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <table>
          <thead>
            <tr><th>Name</th><th>Email</th><th>Role</th><th>Batch ID</th><th>Status</th><th></th></tr>
          </thead>
          <tbody>
            {users.map((u) => (
              <tr
              ...
              >
                <td><strong>{u.name}</strong>{u.id === me?.id && <span className="muted"> (you)</span>}</td>
