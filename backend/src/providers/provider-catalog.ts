...
export const PROVIDER_CATALOG: CatalogProvider[] = [
  ...
  },
  {
    ...
    howToGet:
      'Adapter IMPLEMENTED (People Search API, reverse phone: TYPE_API=api_ppl, F=<phone>, header auth).\nREQUIRED SETUP (all three, or calls are blocked by SearchBug\'s Cloudflare gateway):\n1. Create a PREPAID account at searchbug.com with People Search API access enabled (a free sandbox test account is available first).\n2. AUTHORIZE YOUR SERVER IP: in your SearchBug account API settings, add this server\'s static IP to the allowed-IP list. Datacenter IPs are blocked until whitelisted — this is the #1 cause of a 403.\n3. Copy BOTH your API Key and your CO_CODE (account number). Paste the API Key as API Key and the CO_CODE as API Secret below — both are required on every call.\n4. Record the permitted-use attestation (sales & marketing / lead generation, NOT FCRA/DPPA) and Activate.\nNote: SSN/skip-trace (Professional Trace) needs a separate Restricted-Access approval and is NOT used by this adapter.',
  },
  {
    code: 'TRESTLE',
