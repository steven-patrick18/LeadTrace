...
export const PROVIDER_CATALOG: CatalogProvider[] = [
  ...
  },
  {
    ...
    howToGet:
      'Adapter IMPLEMENTED and format verified against the live API (TYPE=api_ppl, F=<phone>, CO_CODE+PASS query auth).\nREQUIRED SETUP:\n1. Create an account at searchbug.com and load a PREPAID balance — without balance every call returns "Prepaid Plan with balance is required".\n2. If calls come back as a Cloudflare 403 page, ask SearchBug support to allowlist your server IP for API access (support@searchbug.zohodesk.com; already done once for this server).\n3. Copy BOTH your API Key and your CO_CODE (account number). Paste the API Key as API Key and the CO_CODE as API Secret below — both are required on every call.\n4. Record the permitted-use attestation (sales & marketing / lead generation, NOT FCRA/DPPA) and Activate.\nNote: SSN/skip-trace (Professional Trace) needs a separate Restricted-Access approval and is NOT used by this adapter.',
  },
  {
    code: 'TRESTLE',
